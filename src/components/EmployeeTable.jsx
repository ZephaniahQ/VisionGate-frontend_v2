...
export default function EmployeeTable({ records, employees, visitors }) {
  const empPresent  = employees?.filter(e => e.is_present).length ?? 0;
  const empDeparted = employees?.filter(e => !e.is_present && e.entry_time).length ?? 0;
  const empAbsent   = employees?.filter(e => !e.entry_time).length ?? 0;
      ...
      <div className="table-wrap">
        <div className="table-scroll">
          <table>
            ...
            <tbody>
              {records.length === 0 ? (
                <tr><td colSpan="8" className="table-empty">— no employee records today</td></tr>
              ) : records.map(rec => {
                const flags = employeeFlags(rec);
                const dur   = duration(rec.entry_time, rec.exit_time);
                return (
                  <tr key={rec.id}>
                    <td><SnapCell url={rec.snapshot_url} name={rec.name} /></td>
                    <td>
                      <div className="person-id">{rec.id}</div>
                      <div className="person-name">{rec.name}</div>
                    </td>
                    <td><span className="person-dept">{rec.department || "—"}</span></td>
                    <td>
                      {rec.entry_time
                    ...
                    <td>
                      {dur
                        ? <div className="time-val">{dur}</div>
