...
async function fetchWsUrl() {
  const res = await fetch(
    `${SUPABASE_URL}/rest/v1/stream_config?id=eq.1&select=ws_url`,
...
export default function StreamViewer() {
  ...
  const [wsStatus,    setWsStatus]    = useState("fetching"); // fetching | connecting | connected
  ...
  // ── WebSocket — fetch ws_url from Supabase, then connect ────
  useEffect(() => {
    ...
    async function connect() {
      ...
      // Fetch live URL from Supabase (pushed there by Colab after ngrok starts)
      let wsUrl;
      try {
        setWsStatus("fetching");
        wsUrl = await fetchWsUrl();
      } catch (err) {
        console.warn("ws_url fetch failed:", err.message);
        if (wsAlive.current) setTimeout(connect, 5000); // retry — Colab may not be up yet
        return;
      }
      ...
      ws.onopen  = () => { setWsConnected(true); setWsStatus("connected"); };
      ws.onclose = () => {
        setWsConnected(false);
        bboxRef.current = [];
        setDetections([]);
        // Re-fetch on reconnect — ngrok URL changes every Colab restart
  ...
  const wsLabel = wsStatus === "fetching" ? "FETCHING URL" : wsStatus === "connecting" ? "CONNECTING" : "CONNECTED";
