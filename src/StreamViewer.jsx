...
// Added WHEP fetch function
async function fetchWhepUrl(supabaseUrl) {
  const res = await fetch(
    `${supabaseUrl}/rest/v1/stream_config?id=eq.1&select=whep_url`,
    { headers: { apikey: SUPABASE_KEY, Authorization: `Bearer ${SUPABASE_KEY}` } }
  );
  if (!res.ok) throw new Error(`Supabase ${res.status}`);
  const rows = await res.json();
  if (!rows[0]?.whep_url) throw new Error("whep_url empty in database");
  return rows[0].whep_url;
}
...
export default function StreamViewer({
  ...
  onWsMessage,
  onLog,
  whepUrl      = "", // Empty defaults to DB fetch
  wsUrlOverride = "",
  ...
  useEffect(() => {
    ...
    async function startWebRTC() {
      try {
        let finalWhepUrl = whepUrl;

        // Fetch WHEP URL if no override is provided
        if (!finalWhepUrl) {
          log("info", "RTC", "Fetching whep_url from Supabase…");
          try {
            finalWhepUrl = await fetchWhepUrl(supabaseUrl);
            log("ok", "RTC", `Got whep_url: ${finalWhepUrl}`);
          } catch (err) {
        ...
        log("info", "RTC", `Connecting → ${finalWhepUrl}`);
        ...
        const res = await fetch(finalWhepUrl, {
          method: "POST",
